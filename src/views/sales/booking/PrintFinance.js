...
const PrintModal = ({ show, onClose, bookingId }) => {
  ...
  const calculateDownPayment = () => {
    ...
    const disbursement = parseFloat(financeDisbursement) || 0;

    // return dealAmount - (financeDisbursement + gcAmount);

    //return dealAmount - financeDisbursement - exchangeAmount + gcAmount;
    return (dealAmount + gcAmount) - disbursement - exchangeAmount;
  };

  const handleSubmit = async () => {
    try {
      // Validate finance disbursement
      if (!financeDisbursement || financeDisbursement === '') {
        setError('Please enter finance disbursement amount');
        showError('Please enter finance disbursement amount');
        return;
      }

      const disbursementAmount = parseFloat(financeDisbursement);
      if (isNaN(disbursementAmount) || disbursementAmount <= 0) {
        setError('Please enter a valid finance disbursement amount');
        showError('Please enter a valid finance disbursement amount');
        return;
      }
  ...
  const handlePrintFinanceLetter = () => {
    ...
    const disbursementAmount = parseFloat(financeDisbursement) || 0;
    ...
    <>
      ...
      <CModal visible={show} onClose={onClose} size="lg" alignment="center">
        ...
        <CModalFooter className="d-flex justify-content-between">
          <div>
            <CButton color="primary" onClick={handleSubmit} className="me-2" disabled={loading || !financeDisbursement}>
