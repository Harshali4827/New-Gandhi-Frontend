...
//   const [restoreType, setRestoreType] = useState(''); // 'cancelled' or 'rejected_discount'
...
//       // Fetch both booking and cancellation data in parallel
//       await Promise.all([
//         fetchData(),
...
const BookingList = () => {
 ...
 const [isUpdateChassis, setIsUpdateChassis] = useState(false);
  ...
  try {
    ...
    // Add reason as query parameter if it exists
    const queryParams = [];
    
    if (isUpdateChassis && payload.reason) { // Changed from isUpdate to isUpdateChassis
      queryParams.push(`reason=${encodeURIComponent(payload.reason)}`);
    }
    
    // For allocation mode when reason is provided (non-current chassis scenario)
    if (!isUpdateChassis && payload.reason) { // Changed from isUpdate to isUpdateChassis
      queryParams.push(`reason=${encodeURIComponent(payload.reason)}`);
    }
    ...
    console.error(`Error ${isUpdateChassis ? 'updating' : 'allocating'} chassis number:`, error); // Changed from isUpdate to isUpdateChassis
    showError(error.response?.data?.message || `Failed to ${isUpdateChassis ? 'update' : 'allocate'} chassis number`);
  } finally {
    setChassisLoading(false);
  }
