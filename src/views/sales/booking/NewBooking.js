...
//   // Helper function to calculate CGST and SGST
...
//   const calculateDiscountedPrice = (unitPrice, discountValue) => {
//     if (!discountValue || discountValue === '' || isNaN(discountValue)) {
//       return unitPrice;
//     }
...
function BookingForm() {
  ...
  const [formData, setFormData] = useState({
    verticle_id: '', // Added verticle field
    model_id: '',
    model_color: '',
    customer_type: 'B2C',
    rto_type: 'MH',
  ...
  // Filter models based on selected verticle
  const filterModelsByVerticle = (modelsList, verticleId) => {
    if (!verticleId) return modelsList;
  ...
  try {
    ...
    // Get the first verticle from the booking data (since we only select one)
    ...
    setFormData({
      verticle_id: bookingVerticle, // Use the first verticle from array
      model_id: bookingData.model?.id || '',
      model_color: bookingData.color?.id || '',
      customer_type: bookingData.customerType || 'B2C',
      rto_type: bookingData.rto || 'MH',
  ...
  const fetchModels = async (customerType = 'B2C', branchId = null) => {
    try {
      ...
      // Filter models based on selected verticle if one is selected
  ...
  // Prepare request body with verticles as array
  const requestBody = {
    model_id: formData.model_id,
    model_color: formData.model_color,
    customer_type: formData.customer_type,
    rto_type: formData.rto_type,
    branch: formData.branch,
    // =============== IMPORTANT: Send verticles as array ===============
    verticles: formData.verticle_id ? [formData.verticle_id] : [], // Send as array with selected verticle
    // =============== END ===============
    optionalComponents: formData.optionalComponents,
    sales_executive: formData.sales_executive,
    customer_details: {
