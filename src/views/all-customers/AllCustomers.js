...
const AllCustomers = () => {
  ...
  try {
    ...
    // Process each booking to check if chassisNumber exists
    const bookingSummaryRows = ledgerData.bookingWiseSummary
      .map((booking) => {
    ...
    // Calculate totals based on chassisNumber
    let totalDebited = 0;
    let totalBalance = 0;
    
    // Calculate total debited only for bookings with chassisNumber
    ledgerData.bookingWiseSummary.forEach((booking) => {
      const hasChassisNumber = booking.chassisNumber && booking.chassisNumber.trim() !== '';
      if (hasChassisNumber) {
        totalDebited += booking.totalDebited || 0;
        // Also add accessory billing amounts for this booking
    ...
    // Add debit transactions only for bookings with chassisNumber
