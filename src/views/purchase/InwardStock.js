...
function InwardStock() {
  ...
  // FIXED: Update the filteredModels logic
  const filteredModels = models.filter((model) => {
    if (formData.type === 'EV') {
      return model.type === 'EV'; // Only show EV models
    } else if (formData.type === 'ICE') {
      // Show both ICE and CSD models when ICE is selected
      return model.type === 'ICE' || model.type === 'CSD';
    }
    return false; // No type selected, show nothing
  });
